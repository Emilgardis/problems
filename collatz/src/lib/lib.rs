...
pub struct CollatzSieve {
    ...
    pub sieve_data: BTreeMap<u64, Mutex<Option<u64>>>,
    /// Unimplemented.
    ///
    /// FIXME
...
impl CollatzSieve {
    ...
    ///
    /// Can block, but shouldn't.
    pub fn add_result(&mut self, orig: &u64, total_steps: u64) {
        let mut k = self.sieve_data.get(orig).unwrap().lock().unwrap();
        *k = Some(total_steps);
    }
        ...
        match self.sieve.get(val) {
            Some(&(steps, orig)) => {
                match self.sieve_data.get(&orig) {
                    Some(total_steps) => Some(total_steps.lock().unwrap().deref().unwrap() - steps),
...
impl<'a> Iterator for Collatz<'a> {
    ...
    fn next(&mut self) -> Option<u64> {
        ...
        if let Some(steps) = self.sieve
            .as_ref()
            .map_or(None, |ref mut sieve| sieve.in_sieve(&self.curr)) {
        ...
        if self.curr == 1 {
            ...
            if let Some(ref mut sieve) = self.sieve {
                sieve.insert(self.curr, self.count, self.orig)
            };
            self.curr /= 2;
        } else {
            self.count += 1;
...
impl<'a> Collatz<'a> {
    pub fn with_sieve(start: u64, sieve: &'a mut CollatzSieve) -> Collatz {
        sieve.sieve_data.insert(start, Mutex::new(None));
        Collatz {
            orig: start,
            curr: start,
            // walked: vec![],
    ...
    pub fn register(&mut self) {
        if let Some(ref mut sieve) = self.sieve {
            sieve.add_result(&self.orig, self.count)
        };
    }
